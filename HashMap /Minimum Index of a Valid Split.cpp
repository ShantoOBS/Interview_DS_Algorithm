    ...
    int minimumIndex(vector<int>& nums) {
        ...
        int n=nums.size();
         ...
         for(auto u:nums)mp[u]++;
          ...
          for(auto u:mp)if(u.second>maxi){maxi=u.second; dom=u.first;};
          int dom_count=mp[dom];
          int dom_curr=0;
          for(int i=0;i<n;i++){
              ...
              if(dom_curr*2> i+1 && dom_count*2> n-i-1)return i;
          }
        return -1;
    }
};
//Approach-2 (Brute Force using 2 Maps)
//T.C : O(n)
//S.C : O(n)
class Solution {
public:
...
//Approach-3 (Using Majority Element Concept)
//T.C : O(n)
//S.C : O(1)
class Solution {
public:

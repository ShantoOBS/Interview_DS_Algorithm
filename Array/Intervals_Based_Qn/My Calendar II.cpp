...
//My code : Simle and straight forward
...
public:
    ...
    bool book(int start, int end) {
      ...
      for(auto i:d){
       ...
       int  maxi=max(i.first,start);
        int  mini=min(i.second,end);
      ...
      int maxi,mini;
        ...
        for(auto i:v){
             ...
             maxi=max(i.first,start);
             mini=min(i.second,end);

            if(maxi<mini){
               d.emplace_back(maxi,mini);
            }
        }
